...
export default function CustomUnitsScreen() {
  ...
  const [baseRef, setBaseRef] = useState(
    categories.find(c => c.id === activeCat)!.baseUnitId,
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.surface }]}>
      ...
      <View style={styles.rowWrap}>
        {categories.map(cat => (
          <Pressable
            ...
            onPress={() => {
              setActiveCat(cat.id);
              setBaseRef(cat.baseUnitId);
            }}
          >
            <Text
              style={[
                styles.chipText,
                activeCat === cat.id && styles.chipTextActive,
              ]}
            >
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.itemRow}>
            ...
            <Text style={styles.itemMeta}>
              factor {item.factor} offset {item.offset || 0}
            </Text>
            <Pressable
              style={styles.btnSmall}
              onPress={() => remove(item.categoryId, item.id)}
            >
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>{t('customUnits.empty')}</Text>
        }
      />
      ...
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.name')}</Text>
        <TextInput style={styles.input} value={name} onChangeText={setName} />
      </View>
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.symbol')}</Text>
        <TextInput
          style={styles.input}
          value={symbol}
      ...
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.base')}</Text>
        <TextInput
          style={styles.input}
          value={baseRef}
      ...
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.factor')}</Text>
        <TextInput
          style={styles.input}
      ...
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.offset')}</Text>
        <TextInput
          style={styles.input}
          keyboardType="decimal-pad"
          value={offset}
      ...
      <View style={styles.formRow}>
        <Text style={styles.label}>{t('customUnits.notes')}</Text>
        <TextInput
      ...
      <Pressable
        ...
        onPress={() => {
          ...
          Alert.alert(t('customUnits.title'), t('customUnits.saved'));
          setName('');
          setSymbol('');
          setFactor('1');
          setOffset('0');
      ...
      >
        <Text style={styles.btnText}>{t('customUnits.save')}</Text>
      </Pressable>
    </View>
  );
}
